...
function detectIntent(msg: string): string {
  ...
  if (/(كيف أطلب|كيف اطلب|كيف الشراء|أبغى أطلب|ابغى اطلب|طلب خدمة|وش خدماتكم|خدماتكم|أبغى خدمة|ابغى خدمة)/i.test(m)) return 'store_order';

  if (/(أضحية|اضحية|عقيقة|عقيقه|نذر|كفارة|كفاره)/i.test(m)) return 'service_specific';

  if (/(جمعية|جمعيه|خيرية|خيريه|موثق|سجل تجاري|مرخص|سعر|كم السعر|كم يستغرق|مدة التنفيذ|كيف استلم التوثيق|تعديل|إلغاء|الغاء|فتوى|وظائف|وظيفة|توظيف|عمل لديكم|تقبلون طلبات)/i.test(m)) return 'faq';

  if (/(تبرع|تبرعات|أتبرع|ابغى اتبرع|جمعية|خيرية|صدقة|زكاة|إحسان|احسان|غير ربحية|غير هادفة للربح)/i.test(m)) return 'donation';
...
export default function ChatWidget({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (intent === 'faq') {
      ...
      } else if (/(وظائف|وظيفة|توظيف|عمل لديكم|تقبلون طلبات)/i.test(text)) {
        reply = 'حاليًا لا تتوفر معلومات عن وظائف شاغرة عبر هذا المساعد. للاستفسار تواصل معنا عبر واتساب.';
        setMessages(prev => [...prev, {
          role: 'bot',
          text: reply,
          buttons: [{ label: 'واتساب', action: 'wa' }],
    ...
    if (intent === 'donation') {
      setMessages(prev => [...prev, {
        role: 'bot',
        text: `نحن شركة سعودية تجارية مرخصة بسجل تجاري رقم 7052388860، ولسنا جهة خيرية أو غير ربحية، ولا نجمع تبرعات. 🌿

أضحيتي تقوم نيابةً عن عملائها بتنفيذ خدمات الذبائح الشرعية — من أضحية وعقيقة ونذر وكفارة — مع توثيق كامل بالصوت والصورة يصلك بعد التنفيذ مباشرة.

إذا كنت تبحث عن جهة للتبرع، يمكنك التوجه إلى:
- منصة إحسان الرسمية: ihsan.org.sa
- الجمعيات الخيرية المعتمدة في مملكتك`,
        buttons: [
          { label: 'طلب خدمة من المتجر', action: 'store' },
          { label: 'واتساب', action: 'wa' },
        ],
